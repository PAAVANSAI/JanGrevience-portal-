...
import OfficerActions from "@/app/officer/grievances/[id]/OfficerActions";
import OfficerAppealActions from "@/app/officer/grievances/[id]/OfficerAppealActions";
import AssignOfficer from "./AssignOfficer";
import TransferDepartment from "./TransferDepartment";
import InternalNotes from "./InternalNotes";

export default async function DeptAdminGrievanceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!profile || profile.role !== "DEPT_ADMIN") {
    redirect("/");
  }
  ...
  const { data: grievance, error: grievanceError } = await supabase
    ...
    `)
    .eq("id", id)
    .eq("department_id", profile.department_id) // ensure they only see their dept's grievance
    .single();
  ...
  const currentOfficerId = isUnassigned ? null : assignments[0].officer_id;
  const isAssignedToMe = currentOfficerId === user.id;
  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      <main className="max-w-7xl mx-auto p-6 sm:p-8">
        ...
        {/* Header */}
        <div className="bg-surface border border-border rounded-2xl p-6 sm:p-8 mb-6 shadow-sm flex flex-col md:flex-row md:items-start justify-between gap-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Internal Notes */}
            <InternalNotes grievanceId={grievance.id} />
            ...
            {/* Officer Action Panel - Dept Admins can act on grievances if assigned to themselves */}
            <OfficerActions 
              grievanceId={grievance.id}
              currentStatus={grievance.status}
              isAssignedToMe={isAssignedToMe}

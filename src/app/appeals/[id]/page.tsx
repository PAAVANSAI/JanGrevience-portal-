  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        ...
        <Link 
          href="/appeals" 
          className="inline-flex items-center text-sm font-medium text-text-muted hover:text-purple-600 transition-colors mb-6"
        >
        ...
        <div className="bg-surface border border-border rounded-2xl p-6 sm:p-8 mb-6 shadow-sm flex flex-col sm:flex-row sm:items-start justify-between gap-6">
          <div>
            ...
            <p className="text-sm text-text-muted mt-2">
              Submitted on {new Date(appeal.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-surface border border-border rounded-2xl shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-border bg-bg/50">
                <h3 className="text-sm font-bold text-text-primary uppercase tracking-wider">Original Grievance</h3>
              </div>
              <div className="p-6">
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                  <div>
                    <Link href={`/grievances/${g?.id}`} className="text-lg font-bold text-blue hover:underline mb-1 inline-block">
                      {g?.grievance_number}
                    </Link>
                    <p className="font-medium text-text-primary">{g?.subject}</p>
                    <div className="text-sm text-text-muted mt-2 space-y-1">
                      <p>Department: <span className="font-medium text-text-secondary">{g?.departments?.name}</span></p>
          ...
          <div className="lg:col-span-1">
            <div className="bg-surface border border-border rounded-2xl p-6 sm:p-8 shadow-sm sticky top-24">
